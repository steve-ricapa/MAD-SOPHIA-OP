...
def download_findings(token, scan_summary_id, domain='noc'):
    response = requests.get(
...
def download_scans_with_findings(token, scanner_type, domain='noc'):
    ...
    for scan in scans:
        ...
        findings_payload = download_findings(token, scan_summary_id, domain=domain)
...
def main():
    ...
    try:
        download_scans_with_findings(token, scanner_type, domain='noc')
    except Exception as e:
        print(f"Error fetching for noc, trying soc: {e}")
        download_scans_with_findings(token, scanner_type, domain='soc')
