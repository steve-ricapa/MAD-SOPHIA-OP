...
ZABBIX_DIR = Path(__file__).resolve().parents[1]
if str(ZABBIX_DIR) not in sys.path:
    sys.path.insert(0, str(ZABBIX_DIR))
...
def test_send_webhook_requests_upload_url_then_puts_snapshot(monkeypatch):
    ...
    payload = {"scan_id": "ZB-1", "company_id": 4, "scanner_type": "zabbix", "findings": []}
    ...
    assert b'"scan_id":"ZB-1"' in calls["put"]["data"]
